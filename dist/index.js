...
async function getVersion(token, version2) {
  try {
    const octokit = new github.getOctokit(token);
    let release;
    if (tag && tag !== "latest") {
      core.info(`checking GitHub for version '${version2}'`);
    ...
    if (err.status === 404) {
      throw new Error(
        `unable to find '${version2}' - use 'latest' or see https://github.com/${owner}/${repo}/releases for details`
      );
    } else {
      throw err;
    }
...
async function run() {
  try {
    ...
    let version2 = core.getInput("version") || "latest";
    const platform = await getPlatform();
    const arch = await getArch();
    version2 = await getVersion(token, version2);
    const url = `${baseURL}/${version2}/waypoint_${platform}_${arch}.zip`;
